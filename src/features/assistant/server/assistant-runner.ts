...
export async function runAssistant(
  ...
  const result = request.approved
    ? await runConfirmedTool(pending, session)
...
async function continueAssistant(
  ...
  for (let step = 0; step < MAX_STEPS; step += 1) {
    ...
    const response = await requestAssistantCompletion({
      system: buildSystemPrompt(thread, session),
    ...
    const batch = await runToolBatch(response.toolCalls, session);
...
async function runToolBatch(
  ...
  for (const call of toolCalls) {
    ...
    const result = await runTool(tool, parsed.data, session);
...
async function runConfirmedTool(
  ...
  return runTool(tool, parsed.data, session);
}
...
function buildSystemPrompt(
  ...
  return `You are the DG Clicks workspace assistant. You help the signed-in user plan and manage work, and you may act only through the supplied tools.

User: ${session.user.name} (${session.user.email})
Role: ${role}
Current UTC time: ${now}
Current page: ${thread.context.pathname}
Active workspace id: ${thread.context.workspaceId ?? "none"}
Current board id: ${thread.context.boardId ?? "none"}
...
- For multi-step work, briefly state the plan, then execute it in dependency order. Report completed and failed steps truthfully.
